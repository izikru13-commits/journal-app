// Zikkit — השלמת משפט (fill-in-the-blank, vocabulary focus).
function ClozeGame({ profile, onProfileUpdate }) {
  const pool = SENTENCE_BANK.filter((s) => s.skill === "vocab");
  const [item, setItem] = React.useState(() => pickItem(pool, profile, "cloze"));
  ...
  const handleAnswer = (index) => {
    ...
    const { levelChange: change } = finishGame({
      profile, onProfileUpdate, gameId: "cloze", won,
  ...
  const next = () => {
    ...
    setItem(pickItem(pool, profile, "cloze"));
    setSelected(null);
    setAnswered(false);
    setLevelChange(null);
  };
  ...
  return (
    <div className="w-full flex flex-col items-center gap-6" dir="rtl">
      <p className="text-gray-400 text-sm text-center max-w-md">השלימו את המשפט באנגלית עם המילה הנכונה</p>
      <ChoiceQuestion
...
registerGame({ id: "cloze", label: "השלמת משפט", icon: "✏️", category: "vocab", component: ClozeGame });
